...
def insert_data(CompanyCode,DeviceType,Data,Temp,Humid,Info):
   #ทำการเชื่อมต่อกับฐานข้อมูลง่าย ๆ แค่ใส่ Host / User / Password ให้ถูกต้อง
    ...
    sql = "insert into temphumid(CompanyCode,DeviceType,Data,Temp,Humid,Info) values(%s,%s,%s,%s,%s,%s)"
    val=(str(CompanyCode),str(DeviceType),str(Data),str(Temp),str(Humid),str(Info))
                    ...
                    for line_number, line in enumerate(file, start=1):  
                        if word_485 in line:
                            with open('data.text') as fs:
                                ...
                                insert_data(company_data,device,bit_data_Temphumid485,Temp485,Humid485,Info_temp485)
