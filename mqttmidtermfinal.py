...
def insert_data(CompanyCode,DeviceType,Data,Tempsoil,Humidsoil,Infosoil,Statuslight,Datetime,Bitstatuslight):
   #ทำการเชื่อมต่อกับฐานข้อมูลง่าย ๆ แค่ใส่ Host / User / Password ให้ถูกต้อง
    ...
    sql = "insert into plclogo8(CompanyCode,DeviceType,Data,Tempsoil,Humidsoil,Infosoil,Statuslight,Datetime,Bitstatuslight) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    val=(str(CompanyCode),str(DeviceType),str(Data),str(Tempsoil),str(Humidsoil),str(Infosoil),str(Statuslight),str(Datetime),str(Bitstatuslight))
                    ...
                    if word_485 in line:
                        with open('data.text') as fs:
                            ...
                            if client2.connect():
                                ...
                                try:
                                    client2.write_registers(REGISTER_ADDRESS1, data_to_send1)
                                    print("Data sent successfully to address 1 : ", data_to_send1)
                                    client2.write_registers(REGISTER_ADDRESS2, data_to_send2)
                            ...
                            data_control = round(Temp485real)
                            ...
                            insert_data(company_data,device,bit_data_Temphumid485,Temp485soil,Humid485soil,Info_rs485soil,statuslight,strdatetime,bitstatuslight)
